...
conn1 = psycopg2.connect(
   database="test", user='root', password='root', host='127.0.0.1', port= '5432'
)
fake = Faker()
conn = conn1.cursor()
...
for i in range(10):
    sql = f'''INSERT INTO BCustomer VALUES({i+1}, '{fake.name().split()[0]}', '{fake.name().split()[0]}', '{fake.address()}', {randint(20, 40)},'{fake.simple_profile()["sex"]}', {(i+1)*2500}, {randint(1, 10)*3})'''
    conn.execute(sql)
...
for i in range(20):
    ts = int(time.mktime(datetime.datetime.strptime(str(fake.date_time_this_year()), "%Y-%m-%d %H:%M:%S").timetuple()))
